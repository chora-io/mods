...
if ! grep -q "github.com/chora-io/mods/example" go.mod ; then
  echo -e "ERROR: This command must be run from inside the example module."
  return 1
fi
...
mkdir -p proto-tmp/chora/example
cd proto
find . -maxdepth 1 -mindepth 1 -type f -exec cp '{}' ../proto-tmp/ \;
find . -maxdepth 1 -mindepth 1 -type d -exec cp -r '{}' ../proto-tmp/chora/example/ \;
cd ..
...
cp -r github.com/chora-io/mods/example/* ./
rm -rf github.com

...
if ! grep -q "github.com/chora-io/mods/subject" go.mod ; then
  echo -e "ERROR: This command must be run from inside the subject module."
  return 1
fi
...
mkdir -p proto-tmp/chora/subject
cd proto
find . -maxdepth 1 -mindepth 1 -type f -exec cp '{}' ../proto-tmp/ \;
find . -maxdepth 1 -mindepth 1 -type d -exec cp -r '{}' ../proto-tmp/chora/subject/ \;
cd ..
...
cp -r github.com/chora-io/mods/subject/* ./
rm -rf github.com

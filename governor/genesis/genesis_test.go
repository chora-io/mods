...
func setup(t *testing.T, setup func(ctx context.Context, ss governorv1.StateStore)) json.RawMessage {
	ormCtx := ormtable.WrapContextDefault(ormtest.NewMemoryBackend())

	db, err := ormdb.NewModuleDB(&governor.ModuleSchema, ormdb.ModuleDBOptions{})
	require.NoError(t, err)

	ss, err := governorv1.NewStateStore(db)
	require.NoError(t, err)

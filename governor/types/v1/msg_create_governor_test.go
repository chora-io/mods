package v1

import (
	"strconv"
	"strings"
	"testing"

	"github.com/cosmos/gogoproto/jsonpb"
	"github.com/regen-network/gocuke"
	"github.com/stretchr/testify/require"
)

type msgCreateGovernor struct {
	t   gocuke.TestingT
	msg *MsgCreateGovernor
	err error
}

func TestMsgCreateGovernor(t *testing.T) {
	gocuke.NewRunner(t, &msgCreateGovernor{}).
		Path("./msg_create_governor.feature").
		Run()
}

func (s *msgCreateGovernor) Before(t gocuke.TestingT) {
	s.t = t
}

func (s *msgCreateGovernor) Message(a gocuke.DocString) {
	s.msg = &MsgCreateGovernor{}
	err := jsonpb.UnmarshalString(a.Content, s.msg)
	require.NoError(s.t, err)
}

func (s *msgCreateGovernor) MetadataWithLength(a string) {
	length, err := strconv.ParseInt(a, 10, 64)
	require.NoError(s.t, err)

	s.msg.Metadata = strings.Repeat("x", int(length))
}

func (s *msgCreateGovernor) ValidateMessage() {
	s.err = s.msg.ValidateBasic()
}

func (s *msgCreateGovernor) ExpectNoError() {
	require.NoError(s.t, s.err)
}

func (s *msgCreateGovernor) ExpectTheError(a gocuke.DocString) {
	require.EqualError(s.t, s.err, a.Content)
}

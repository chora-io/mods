...
if ! grep -q "github.com/chora-io/mods/governor" go.mod ; then
  echo -e "ERROR: This command must be run from inside the governor module."
  return 1
fi
...
mkdir -p proto-tmp/chora/governor
cd proto
find . -maxdepth 1 -mindepth 1 -type f -exec cp '{}' ../proto-tmp/ \;
find . -maxdepth 1 -mindepth 1 -type d -exec cp -r '{}' ../proto-tmp/chora/governor/ \;
cd ..
...
cp -r github.com/chora-io/mods/governor/* ./
rm -rf github.com

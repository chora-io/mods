...
if ! grep -q "github.com/chora-io/mods/ecosystem" go.mod ; then
  echo -e "ERROR: This command must be run from inside the ecosystem module."
  return 1
fi
...
mkdir -p proto-tmp/chora/ecosystem
cd proto
find . -maxdepth 1 -mindepth 1 -type f -exec cp '{}' ../proto-tmp/ \;
find . -maxdepth 1 -mindepth 1 -type d -exec cp -r '{}' ../proto-tmp/chora/ecosystem/ \;
cd ..
...
cp -r github.com/chora-io/mods/ecosystem/* ./
rm -rf github.com

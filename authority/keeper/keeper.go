...
type Keeper struct {
	...
	ss authorityv1.StateStore // module state store
}
...
func NewKeeper(storeService store.KVStoreService) Keeper {
	...
	var err error
	k.db, err = ormdb.NewModuleDB(&authority.ModuleSchema, ormdb.ModuleDBOptions{
	...
	k.ss, err = authorityv1.NewStateStore(k.db)
	if err != nil {
		panic(err)
	}
